...
def adding_category_mutation(data_mutational,gene_name,hgsvp_short,variant_classification,chromosome,start,end):
    ...
    if hgsvp_short=="" or hgsvp_short==None:
        ...
        data_mutational['mutation_label'] = data_mutational.apply(lambda row: nuovi_nomi[(row[f'{gene_name}'], row[f'{variant_classification}'],row[f'{chromosome}'],row[f'{start}'],row[f'{end}'])], axis=1)
        ...
        data_mutational['mutation_label'] = data_mutational.apply(lambda row: nuovi_nomi[(row[f'{gene_name}'], row[f'{hgsvp_short}'], row[f'{variant_classification}'],row[f'{chromosome}'],row[f'{start}'],row[f'{end}'])], axis=1)
        return data_mutational
...
def create_maps(data_mutational,data_config,column_mutation,gene_interest=""):
    ...
    #GET INFOS FROM DATA
        ...
        if column_mutation=="":
            ...
            _category=str(_row[1]["mutation_label"])
...
def centroids_cluster(dendro,path_save):
    with open (f"{path_save}/Centroidi_Mutazioni.csv","w") as f:
        for _i in range(len(dendro)):
            ...
            #print(path_save)
            f.write(f"Cluster {_i} centroids found {len(_list_centroids)}: {_list_centroids}\n")
...
#FIXME usa il grafo per ritrovare il gene
...
def oncoplot(MAP_PATIENTS,MAP_VARIANTS,cluster_more_patient,path_saved):
    ...
    os.makedirs(f"{path_saved}/oncoplot",exist_ok=True)
