...
def draw_ellipse(_, ax, x, y, w, h, a, fillcolor) -> None:
    e = patches.Ellipse(xy=(x, y), width=w, height=h, angle=a, color=fillcolor)
    ax.add_patch(e)


def draw_triangle(_, ax, x1, y1, x2, y2, x3, y3, fillcolor) -> None:
    xy = [
        (x1, y1),
        (x2, y2),
        (x3, y3),
    ]
    polygon = patches.Polygon(xy=xy, closed=True, color=fillcolor)
...
def venn6(labels, names=None, **options):
